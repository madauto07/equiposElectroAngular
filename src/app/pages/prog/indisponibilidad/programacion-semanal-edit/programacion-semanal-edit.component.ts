...
@Component({
  selector: 'app-programacion-semanal-edit',
  templateUrl: './programacion-semanal-edit.component.html',
  styles: [
  ]
})
export class ProgramacionSemanalEditComponent implements OnInit {
  ...
  programacionTecSemanalModel: ProgramacionTecnicaSemanalModel = new ProgramacionTecnicaSemanalModel();
  ...
  obtenerDatos(): void {
    if (this.idProgramacion > 0) {
      this.programacionTecnicaSemanalService.obtener(this.idProgramacion).subscribe((result) => {
        this.programacionTecSemanalModel = result;
        console.log(result);
        this.titulo=result.strNombre;
        this.labelanho=result.intAnho.toString();
        this.labelSem=result.intSemana.toString();
  ...
  listar(indicePagina = 0, numeroFilasABuscar = this.numeroFilas): void {
    ...
    .listarPageDetalle(2022,0,0,
      ...
      this.form1.value.filtroCodigoEquipo,3)
